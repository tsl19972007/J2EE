...
public class SubmitOrder extends HttpServlet {
    ...
    public SubmitOrder(){
        ...
        orderServiceImpl=(OrderService)EJBFactory.getEJB("OrderServiceBean","service.OrderService");
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(balance<order.getOrderSum()){
            response.getWriter().print("http://localhost:8000/ServletTest/SubmitOrder?result=fail");
        }else {
            saveOrder(order);
            clearShoppingCart(request, response);
            if(order.getOrderSum()>66){
                int discount=(int)order.getOrderSum()/10;
                response.getWriter().print("http://localhost:8000/ServletTest/SubmitOrder?result=success&discount="+discount);
            }else{
                response.getWriter().print("http://localhost:8000/ServletTest/SubmitOrder?result=success");
            }
        }
    ...
    private void displaySubmitResult(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
        ...
        out.println("<a href='http://localhost:8000/ServletTest/ShowHomePage'>返回</a>");
